...
CREATE TABLE tournament (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  title         TEXT NOT NULL,
  max_bracket_size INT CHECK (max_bracket_size IN (128,256)),
...
CREATE TYPE submission_status AS ENUM ('draft','submitted','locked','disqualified');
...
CREATE INDEX idx_eval_round ON evaluation(round_id);
CREATE INDEX idx_like_track ON track_like(match_track_id);
CREATE INDEX idx_comment_match ON comment(match_id);

CREATE VIEW qualifier2_totals AS
SELECT
  s.participant_id,
  s.round_id,
...
FROM submission s
JOIN evaluation e
  ON e.target_type='submission' AND e.target_id = s.id
GROUP BY s.participant_id, s.round_id;

CREATE VIEW match_totals AS
SELECT
  m.id AS match_id,
  AVG(e.total_score)::NUMERIC(5,2) AS avg_score
FROM match m
JOIN evaluation e
  ON e.target_type='match' AND e.target_id = m.id
GROUP BY m.id;
